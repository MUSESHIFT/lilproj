...
const getBusinessName = createServerFn({ method: "GET" }).handler(async () => {
  try {
    ...
    return cfg.businessName?.trim() ?? "money rundown";
  } catch {
    return "money rundown";
  }
});
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-6 text-lg leading-relaxed text-gray-600 sm:text-xl">
            money rundown connects to your email inbox, automatically finds every invoice and
  ...
  const steps = [
    {
      ...
      description:
        "Link your Gmail account with one click. money rundown works in the background — no setup, no folder rules, no manual tagging.",
      icon: (
...
function Features() {
  const features = [
    ...
    },
    {
      ...
      description:
        "No forms, no uploads, no spreadsheets. money rundown works entirely from your inbox — the data is already there.",
      icon: (
        <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center justify-between gap-6 sm:flex-row">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold text-gray-900">money rundown</span>
          </div>
          ...
          <p className="text-sm text-gray-400">
            &copy; {new Date().getFullYear()} money rundown. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
